...
## Major Comments
1. There is still confusion about what the abbreviations “CNV” and “CNVs” stand for. The authors introduce in the first line of the Abstract “CNV” to abbreviate “copy number variation” – a term that has no plural. Later in the abstract, the authors use “CNVs” without introducing this abbreviation. The reader must assume the authors use “CNVs” to refer to “copy number variations”, but I think common practice is to call what the authors mean here “copy number variants”. I suggest that the authors define and use “CNV” and “CNVs” to mean “copy number variant” and “copy number variations”, respectively. I would not introduce an abbreviation for “copy number variation”.
2. I did not understand the authors' argument in the second-to-last paragraph of their response to Major Issue 6 by reviewer 1. Reviewer 1 seemed to be concerned about $V_{\mathrm{ST}}$ is being inflated if within-population copy number variation is low. However, the authors seem to argue that because within-population copy number variation is low, and so its influence on $V_{\mathrm{ST}}$ would be minor. Why that? At least when drawing from the analogy to $F_{\mathrm{ST}}$, low within-population variation is prone to inflating $V_{\mathrm{ST}}$. In this context, I find it unsatisfactory that the authors do not explicitly define how $V_{\mathrm{ST}}$ is calculated. The authors' scripts are not freely accessible.
3. I was not able to access the script files from the GitLab repository at the LRZ center for computation. I therefore could not (spot) check the code. I consider this a limitation that should be fixed in the process of peer review if the journal wants the review to cover the supplementary code files. Multiple URLs given to specific sections within the GitLab repository are given throughout the Methods section, but none of them leads to content that is publicly accessible.
...
- Fig. 1:
    ...
    - [l.910] **C/S:** I was confused about the TGRC populations being mentioned here. What is the difference between the black dots and the coloured dots? The Methods section is missing a clear description of the sampling design. 
- Fig. 2:
    - **C:** ...
- Fig. 4:
    - **C:** ...
...
### Tables
- Table 1:
    - **C/S:** I am unclear about what the columns “Number of CN gained” and “Number of CN lost” mean. Are these the numbers of gene copies gained summed over all expanding and contracting genes, respectively?
